...
export default function SidebarLayers() {
  ...
  // SidebarLayers.js
  const renderLayerItems = (currentBlocksList: any, level = 0) => {
    return currentBlocksList?.map((block: any, index: number) => (
      <div
        key={block.id}
      ...
      >
        ...
        >
          <div role="button">
            <LayerItem
              block={block}
              blockIcon={blocksList[block.type]?.icon || null}
  ...
  return (
    <Sheet>
      ...
      <SheetContent position="bottom" className={navbarContent}>
        ...
        <div className="mx-auto flex max-h-[360px] w-full max-w-3xl flex-col items-center justify-between overflow-y-auto">
          {structure?.length === 0 ? (
            <div className="p-4">
